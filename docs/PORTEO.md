...
| `SoporteCania` (uno por banda, sockets `Gear*`) | El patrón del gancho aplicado a la caña: `Zona` para la mira, ocupar/liberar, y `set_doblado()` — la caña clavada le presta su muelle real y el soporte es su cuerpo visible. El sedal sale de su `Punta`. |
